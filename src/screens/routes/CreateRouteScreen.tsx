...
import { createAndPublishRoute, CreateWaypointData } from '@/services/createRoute';
...
const WAYPOINT_TYPES: { value: WaypointType; label: string; icon: keyof typeof Ionicons.glyphMap }[] = [
...
export default function CreateRouteScreen() {
    ...
    const [selectedWaypointType, setSelectedWaypointType] = useState<WaypointType>('otro');
    ...
    const handleMapPress = (event: any) => {
        ...
        if (isAddingWaypoint) {
            ...
            if (!waypointName.trim()) {
                Alert.alert('Nombre requerido', 'Por favor ingresa un nombre para el punto de interés');
                return;
            }
                    ...
                    <React.Fragment key={step.key}>
                        <View style={[
                            ...
                            {isPast ? (
                                ...
                                <Ionicons name={step.icon} size={14} color={isActive ? neutral.white : neutral.gray400} />
                            )}
                        </View>
    ...
    const renderDrawStep = () => (
        <>
                ...
                {routeCoordinates.map((coord, index) => (
                    <Marker key={index} coordinate={coord}>
                        <View style={[
                            ...
                            index === routeCoordinates.length - 1 && index > 0 && styles.endPoint,
                        ]}>
            ...
            <View style={styles.drawControls}>
                <TouchableOpacity
                    style={[styles.drawButton, routeCoordinates.length === 0 && styles.drawButtonDisabled]}
                    onPress={handleUndo}
                    disabled={routeCoordinates.length === 0}
                >
                    <Ionicons name="arrow-undo" size={20} color={routeCoordinates.length === 0 ? neutral.gray400 : neutral.gray700} />
                </TouchableOpacity>
                <TouchableOpacity
                    style={[styles.drawButton, routeCoordinates.length === 0 && styles.drawButtonDisabled]}
                    onPress={handleClear}
                    disabled={routeCoordinates.length === 0}
                >
                    <Ionicons name="trash-outline" size={20} color={routeCoordinates.length === 0 ? neutral.gray400 : semantic.error} />
                </TouchableOpacity>
            </View>
            ...
            <View style={styles.infoOverlay}>
                ...
                <View style={styles.statsRow}>
                    <View style={styles.statItem}>
                        ...
                        <Text style={styles.statText}>{routeCoordinates.length} puntos</Text>
                    </View>
                    <View style={styles.statItem}>
        ...
        <>
            ...
            {isAddingWaypoint ? (
                <View style={styles.waypointInputOverlay}>
                    <Text style={styles.waypointInputTitle}>Toca el mapa para ubicar el punto</Text>
                    <TextInput
                    ...
                    />
                    <ScrollView horizontal showsHorizontalScrollIndicator={false} style={styles.waypointTypesScroll}>
                        {WAYPOINT_TYPES.map(type => (
                            <TouchableOpacity
                                key={type.value}
                                style={[
                                    styles.waypointTypeChip,
                                    selectedWaypointType === type.value && styles.waypointTypeChipActive,
                                ]}
                            ...
                            >
                                <Ionicons
                                    name={type.icon}
                                    size={16}
                                    color={selectedWaypointType === type.value ? neutral.white : neutral.gray700}
                                />
                                <Text style={[
                                    styles.waypointTypeText,
                                    selectedWaypointType === type.value && styles.waypointTypeTextActive,
                                ]}>
                                    {type.label}
                                </Text>
                            </TouchableOpacity>
                    ...
                    <TouchableOpacity style={styles.cancelWaypointButton} onPress={() => setIsAddingWaypoint(false)}>
                        <Text style={styles.cancelWaypointText}>Cancelar</Text>
                    </TouchableOpacity>
                </View>
            ) : (
                <View style={styles.waypointsListOverlay}>
                    <Text style={styles.waypointsTitle}>Puntos de Interés ({waypoints.length})</Text>
                    {waypoints.length === 0 ? (
                        <Text style={styles.noWaypointsText}>No hay puntos de interés aún</Text>
                    ) : (
                            ...
                            {waypoints.map((wp, index) => (
                                <View key={index} style={styles.waypointItem}>
                                    <Ionicons
                                        ...
                                        color={brand.primary}
                                    />
                                    <Text style={styles.waypointItemName} numberOfLines={1}>{wp.name}</Text>
                    ...
                    <TouchableOpacity style={styles.addWaypointButton} onPress={() => setIsAddingWaypoint(true)}>
        ...
        <View style={[styles.container, { paddingTop: insets.top }]}>
            ...
            {currentStep !== 'publishing' && (
                <View style={[styles.bottomActions, { paddingBottom: insets.bottom + 16 }]}>
                    {currentStep === 'details' ? (
                        <TouchableOpacity
                            style={[styles.primaryButton, !routeName.trim() && styles.primaryButtonDisabled]}
                        ...
                        <TouchableOpacity
                            style={[
                                ...
                                currentStep === 'draw' && routeCoordinates.length < 2 && styles.primaryButtonDisabled,
                            ]}
                            onPress={handleNext}
